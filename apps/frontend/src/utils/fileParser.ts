...
/**
 * 为 LlamaIndex 后端准备文件数据
 * 返回纯文本内容，不包含元数据包装
 */
export async function parseFileForLlamaIndex(file: File): Promise<string> {
  // 检查文件大小
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(`文件大小超过限制 (${Math.round(MAX_FILE_SIZE / 1024 / 1024)}MB)`)
  }
  ...
  try {
    logger.info(
      'Starting file parsing for LlamaIndex',
    ...
    logger.info(
      'File parsing completed for LlamaIndex',
      { fileName: file.name, contentLength: content.length },
      'FileParser'
    )

    return content.trim()
  } catch (error) {
    logger.error('File parsing failed for LlamaIndex', error, 'FileParser')
    throw error instanceof Error ? error : new Error('文件解析失败')
  }
}

/**
 * 为 LlamaIndex 后端准备完整的文件上传数据
 */
export async function prepareFileForUpload(file: File): Promise<{
  ...
  try {
    ...
    logger.error('Failed to prepare file for upload', error, 'FileParser')
    throw error
  }
}
