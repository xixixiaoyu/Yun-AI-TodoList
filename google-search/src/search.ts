...
import { CommandOptions, HtmlResponse, SearchResponse, SearchResult } from './types.js'
// import { url } from "inspector"; // 未使用的导入
...
export async function googleSearch(
  ...
  if (fs.existsSync(stateFile)) {
    ...
    if (fs.existsSync(fingerprintFile)) {
      try {
        ...
        logger.warn({ error: _e }, '无法加载指纹配置文件，将创建新的指纹')
      }
    }
  } else {
  ...
  // 获取随机设备配置或使用保存的配置
  const getDeviceConfig = (): [string, Record<string, unknown>] => {
  ...
  async function performSearch(headless: boolean): Promise<SearchResponse> {
    ...
    await context.addInitScript(() => {
      ...
      if (typeof window !== 'undefined' && 'WebGLRenderingContext' in window) {
        const WebGLContext = (window as unknown as Record<string, unknown>)
          .WebGLRenderingContext as { prototype: { getParameter: Function } }
        const getParameter = WebGLContext.prototype.getParameter
        WebGLContext.prototype.getParameter = function (parameter: number) {
    ...
    try {
      ...
      const isBlockedPage = sorryPatterns.some(
        (pattern) =>
          currentUrl.includes(pattern) || (response && response.url().toString().includes(pattern))
      )
        ...
        if (headless) {
          ...
          if (browserWasProvided) {
            ...
            try {
              ...
              const _tempPage = await tempContext.newPage()
      ...
      results = await page.evaluate((maxResults: number): SearchResult[] => {
        ...
        for (const selectors of selectorSets) {
          ...
          for (const container of containers) {
            ...
            if (linkInTitle) {
              ...
              let current = titleElement as HTMLElement | null
              while (current && current.tagName !== 'A') {
                current = current.parentElement
              }
              if (current && current.tagName === 'A') {
                link = (current as { href: string }).href
              } else {
        ...
        if (results.length < maxResults) {
          ...
          for (const el of anchorElements) {
            ...
            const link = (el as { href: string }).href
            // 过滤掉导航链接、图片链接、已存在链接等
            if (
              !link ||
              seenUrls.has(link) ||
...
export async function getGoogleSearchPageHtml(
  ...
  async function performSearchAndGetHtml(headless: boolean): Promise<HtmlResponse> {
    ...
    try {
      ...
      // 返回错误信息
      throw new Error(
        `获取Google搜索页面HTML失败: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
