...
program
  ...
  .action(
    async (
      query: string,
      options: CommandOptions & { getHtml?: boolean; saveHtml?: boolean; htmlOutput?: string }
    ) => {
      try {
        if (options.getHtml) {
          // 获取HTML
          ...
          if (options.saveHtml && htmlResult.savedPath) {
            console.error(`HTML已保存到文件: ${htmlResult.savedPath}`)
          }
          ...
          const outputResult = {
            ...
            // 只输出HTML的前500个字符作为预览
            htmlPreview:
              htmlResult.html.substring(0, 500) + (htmlResult.html.length > 500 ? '...' : ''),
          }

          console.error(JSON.stringify(outputResult, null, 2))
        } else {
          // 执行常规搜索
